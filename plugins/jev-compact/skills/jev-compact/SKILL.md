...
When installed through the Jev Compact Codex plugin, lifecycle hooks create
sidecar checkpoints automatically before native compaction and after large
tool-heavy turns. They do not replace or shrink the active Codex transcript.
Read [automatic Codex checkpoints](references/automatic-codex.md) when checking,
recovering, or configuring those artifacts.
