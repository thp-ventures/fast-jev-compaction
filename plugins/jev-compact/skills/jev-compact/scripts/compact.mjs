...
export async function run(value, output, transport = sendRequest) {
  await checkPlan(value);
  ...
  try {
    ...
    for (let i = 0; i < value.requests.length; i += 4) {
      const batch = await Promise.allSettled(value.requests.slice(i, i + 4).map(body => transport(body, credential)));
...
async function sendRequest(body, credential) {
...
export async function main(args) {
  if (args.length === 1 && args[0] === 'status') {
    ...
    const report = await run(read(args[1]), args[2], sendRequest);
