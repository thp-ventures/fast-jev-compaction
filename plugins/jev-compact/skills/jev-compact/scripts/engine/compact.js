...
function truncatedResultText(text, isError, headChars) {
    ...
    return `${head}[fast-jev-compaction truncated ${text.length - headChars} chars of this tool result${isError ? ' (error)' : ''}; consult the original transcript; do not repeat a side-effecting call]`;
}
/**
...
export function applyDecisions(messages, decisions, calls, headChars) {
    ...
    for (const decision of decisions) {
        ...
        if (call && !call.pinned && decision.action !== 'keep')
