 ...
 */
export function collectToolCalls(messages, preserveRecentMessages, protection = {}) {
    ...
    for (const id of protectedIds) {
        if (!knownIds.has(id))
            throw new Error(`Unknown protected tool_use_id: ${id}`);
    }
    ...
    messages.forEach((message, callIndex) => {
        for (const tool of message.toolUses) {
            ...
            calls.push({
                ...
                pinned: protectedIds.has(tool.tool_use_id) ||
